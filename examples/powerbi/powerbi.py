...
class PowerBI(Blueprint):
    ...
    refreshable_datasets = Output(datasets, lambda datasets: [dataset for dataset in datasets if dataset["isRefreshable"]])
    refreshable_dataset_id = Output(refreshable_datasets, lambda dataset: dataset["id"], loop=True)

    refreshes = Node(api.refreshes_request)
    refresh_schedule = Node(api.refresh_schedule_request)

    reports = Node(api.reports_request)
    powerbi_reports = Output(reports, lambda reports: [report for report in reports if report["reportType"] == "PowerBIReport"])
    powerbi_report_id = Output(powerbi_reports, lambda report: report["id"], loop=True)
    paginated_reports = Output(reports, lambda reports: [report for report in reports if report["reportType"] == "PaginatedReport"])
    paginated_report_id = Output(paginated_reports, lambda report: report["id"], loop=True)
